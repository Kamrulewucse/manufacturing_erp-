@extends('layouts.app')
@section('title')
    Edit Cheque Receipt(CR) # <b>{{ $receiptPayment->receipt_payment_no }}</b>
@endsection
@section('style')
    <style>
        .form-control {
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="card card-outline card-default">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form enctype="multipart/form-data" action="{{ route('cheque_receipt.edit',['receiptPayment'=>$receiptPayment->id]) }}" class="form-horizontal" method="post">
                    @csrf
                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group {{ $errors->has('financial_year') ? 'has-error' :'' }}">
                                                    ...
                                                    <input type="text" name="financial_year" readonly value="{{ $receiptPayment->financial_year }}" class="form-control">
                                        ...
                                        <div class="row hide client_name_area">
                                            <div class="col-md-6">
                                                <div class="form-group {{ $errors->has('payee_name') ? 'has-error' :'' }}">
                                                    <label for="payee_name">Depositor Name <span class="text-danger">*</span></label>
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group {{ $errors->has('cheque_date') ? 'has-error' :'' }}">
                                                    <label for="cheque_date">Cheque Date <span
                                                            class="text-danger">*</span></label>
                                                    <input type="text" value="{{ old('cheque_date',\Carbon\Carbon::parse($receiptPayment->cheque_date)->format('d-m-Y')) }}" id="cheque_date" name="cheque_date" class="form-control date-picker" placeholder="Enter Cheque Date">
                                                    @error('cheque_date')
                                                    <span class="help-block">{{ $message }}</span>
                                                    @enderror
                                                </div>
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    ...
                                                    <div class="form-group {{ $errors->has('notes') ? 'has-error' :'' }}">
                                                        <input type="text" value="{{ old('notes',$receiptPayment->notes) }}" name="notes" class="form-control" placeholder="Enter Receipt Details (Narration)...">
                                                        @error('notes')
                                                        <span class="help-block">{{ $message }}</span>
                                                        @enderror
                                                    </div>
...
@section('script')
    <script>
        $(function (){
            ...
            {{--        $.ajax({--}}
            {{--            method: "GET",--}}
            {{--            url: "{{ route('accounts.check_fixed_asset') }}",--}}
            ...
            {{--                //accountHeadItem.closest('.receipt-payment-item').find('.deprecation_percentage').attr("", "true");--}}
            {{--            }else{--}}
            {{--                accountHeadItem.closest('.receipt-payment-item').find('.deprecation_area').hide();--}}
            {{--                //accountHeadItem.closest('.receipt-payment-item').find('.deprecation_percentage').attr("", "false");--}}
            {{--            }--}}
