        ...
        <div class="col-md-12">
            ...
            <div class="card card-outline card-default">
                <div class="card-header">
                    <h3 class="card-title">Section Number of TDS Information</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form enctype="multipart/form-data" action="{{ route('section_number_of_tds.add') }}" class="form-horizontal" method="post">
                    @csrf
                    <div class="card-body">
                        <div class="form-group row {{ $errors->has('source') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-9">
                                <input type="text" value="{{ old('source') }}" name="source" class="form-control" id="source" placeholder="Enter Source">
                                @error('source')
                        ...
                        <div class="form-group row {{ $errors->has('sort') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-9">
                                <input type="text" value="{{ old('sort',$maxSection + 1) }}" name="sort" class="form-control" id="sort" placeholder="Enter Sort">
                                @error('sort')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
