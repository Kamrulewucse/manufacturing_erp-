...
class CashVoucherController extends Controller
{
    public function datatable()
    {
        ...
        return DataTables::eloquent($query)
            ->addColumn('action', function (ReceiptPayment $receiptPayment) {
                $btn = '';
                $btn .= ' <a target="_blank" href="' . route('cash_voucher_print', ['receiptPayment' => $receiptPayment->id]) . '" class="btn btn-dark btn-sm"><i class="fa fa-print"></i></a> ';
                $btn .= ' <a href="' . route('cash_voucher_details', ['receiptPayment' => $receiptPayment->id]) . '" class="btn btn-dark btn-sm"><i class="fa fa-info-circle"></i></a> ';
                if ($receiptPayment->is_delete == 0)
                    $btn .= ' <a href="' . route('cash_voucher.edit', ['receiptPayment' => $receiptPayment->id]) . '" class="btn btn-dark btn-sm"><i class="fa fa-edit"></i></a> ';
    ...
    {
        return view('accounts.cash_voucher.all');
    }
    public function details(ReceiptPayment $receiptPayment)
    {
        ...
        return view('accounts.cash_voucher.details',compact('receiptPayment','inWordAmount'));
    }
    public function print(ReceiptPayment $receiptPayment)
    {
        ...
        return view('accounts.cash_voucher.print',compact('receiptPayment','inWordAmount'));
    }
    public function rangePrint(Request $request)
    {
        ...
        $from = 'CV-'.$request->from.'-'.$selectCash->account_code;
        $to = 'CV-'.$request->to.'-'.$selectCash->account_code;

        $voucherLists = [];
        for($i = $request->from;$i <= $request->to;$i++){
            array_push($voucherLists,'CV-'.$i.'-'.$selectCash->account_code);
        }

        $receiptPayments = ReceiptPayment::where('sister_concern_id',auth()->user()->sister_concern_id)
            ->where('payment_account_head_id',$request->cash_account_code)
        ...
        return view('accounts.cash_voucher.range_print',compact('receiptPayments',
    ...
    public function create()
    {
        ...
        return view('accounts.cash_voucher.add',compact('taxSections','projects'));
    }
    ...
    public function createPost(Request $request)
    {
        ...
        if ($request->account_head_code) {
            ...
            return redirect()
                ->route('accounts.cash_voucher.create')
                ->withInput()
        ...
        return redirect()->route('cash_voucher_details', ['receiptPayment' => $receiptPayment->id])
            ->with('message', 'Cash Voucher(CB) created');
    ...
    public function edit(ReceiptPayment $receiptPayment)
    {
        ...
        $taxSections =TaxSection::orderBy('sort')->get();
        return view('accounts.cash_voucher.edit',compact('receiptPayment', 'fiscalYear','taxSections'));
    }
    public function editPost(ReceiptPayment $receiptPayment, Request $request)
    {
        ...
        return redirect()->route('cash_voucher_details', ['receiptPayment' => $receiptPayment->id])
            ->with('message', 'Cash Voucher(CB) Updated');
