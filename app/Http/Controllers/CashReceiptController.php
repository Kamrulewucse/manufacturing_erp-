...
class CashReceiptController extends Controller
{
    public function datatable() {
        $query = ReceiptPayment::select('receipt_payments.*')
            ->where('receipt_payments.transaction_type', 1)
            ->where('receipt_payments.payment_type', 2)
        ...
        return DataTables::eloquent($query)
            ->addColumn('action', function(ReceiptPayment $receiptPayment) {
                $btn = '';
                $btn .= '<a target="_blank" href="'.route('cash_receipt_print',['receiptPayment'=>$receiptPayment->id]).'" class="btn btn-dark btn-sm"><i class="fa fa-print"></i></a> <a href="'.route('cash_receipt_details',['receiptPayment'=>$receiptPayment->id]).'" class="btn btn-dark btn-sm"><i class="fa fa-info-circle"></i></a> ';
                if ($receiptPayment->is_delete == 0 )
                    $btn .= ' <a href="'.route('cash_receipt.edit',['receiptPayment'=>$receiptPayment->id]).'" class="btn btn-dark btn-sm"><i class="fa fa-edit"></i></a>';

                return $btn;
            })
            ->addColumn('incomes_code', function(ReceiptPayment $receiptPayment) {
            ...
            ->editColumn('net_total', function(ReceiptPayment $receiptPayment) {
            ...
            ->rawColumns(['action','incomes_code'])
            ->toJson();
    }

    public function index()
    {
        return view('accounts.cash_receipt.all');
    }
    public function details(ReceiptPayment $receiptPayment)
    {
        $inWordAmount = new DecimalToWords();
        return view('accounts.cash_receipt.details',compact('receiptPayment','inWordAmount'));
    }

    public function print(ReceiptPayment $receiptPayment)
    {
        $inWordAmount = new DecimalToWords();
        return view('accounts.cash_receipt.print',compact('receiptPayment','inWordAmount'));
    }

    public function rangePrint(Request $request)
    {
        $selectCash = AccountHead::find($request->cash_account_code);
        if (!$selectCash){
            abort('404','Cash not found!');
        }

        $from = 'CMR-'.$request->from.'-'.$selectCash->account_code;
        $to = 'CMR-'.$request->to.'-'.$selectCash->account_code;
        $voucherLists = [];
        for($i = $request->from;$i <= $request->to;$i++){
            array_push($voucherLists,'CMR-'.$i.'-'.$selectCash->account_code);
        }
        $receiptPayments = ReceiptPayment::where('payment_account_head_id', $request->cash_account_code)
        ...
        $inWordAmount = new DecimalToWords();
        return view('accounts.cash_receipt.range_print',compact('receiptPayments',
    ...
    public function create()
    {
        ...
        return view('accounts.cash_receipt.add',compact('taxSections','projects'));
    }
    ...
    public function createPost(Request $request)
    {
        ...
        if ($request->client_type == 1){
            $rules['depositor'] = 'required';

        }else{
            $rules['depositor_name']= 'required|unique:clients,name|max:255';
        ...
        if ($request->account_head_code){
            ...
            $grandTotal = $totalAmount - $totalOtherAmount;
        }
        ...
        $cashAccount = AccountHead::where('id', $request->cash_account_code)
            ->first();
        $receiptPayment->payment_account_head_id = $request->cash_account_code;

        if ($request->client_type == 1) {
            $accountHead = AccountHead::find($request->depositor);
        } else {
            ...
            $client->name = $request->depositor_name;
        ...
        foreach ($request->account_head_code as $reqAccountHeadCode){
            ...
            $receiptPaymentDetail->net_total = $request->amount[$counter];
        ...
        if ($request->other_account_head_code){
            ...
            foreach ($request->other_account_head_code as $reqOtherAccountHeadCode){
                ...
                //Expense Other Head Amount
                $log = new TransactionLog();
                $log->payment_account_head_id = $receiptPayment->payment_account_head_id;
                $log->payee_depositor_account_head_id = $accountHead->id;
                $log->transaction_type = 18;
                $log->receipt_payment_no = $voucherNo;
                $log->receipt_payment_sl = $receiptPaymentNoSl;
                $log->financial_year = financialYear($request->financial_year);
                $log->other_head = 1;
        ...
        return redirect()->route('cash_receipt_details',['receiptPayment'=>$receiptPayment->id])
                ->with('message','Cash Receipt(CR) created');
    ...
    public function edit(ReceiptPayment $receiptPayment)
    {
        ...
        $taxSections =TaxSection::orderBy('sort')->get();
        return view('accounts.cash_receipt.edit',compact('receiptPayment',
        'fiscalYear','taxSections'));
    }
    public function editPost(ReceiptPayment $receiptPayment,Request $request)
    {
        $rules = [
            ...
            'e_tin'=>'nullable|max:255',
            'address'=>'nullable|max:255',
            'nature_of_organization'=>'nullable',
            'designation'=>'nullable|max:255',
            'account_head_code.*'=>'required',
            'amount.*'=>'required|numeric',
            'notes'=>'nullable|max:255',
        ...
        return redirect()->route('cash_receipt_details',['receiptPayment'=>$receiptPayment->id])
            ->with('message','Cash Receipt(CR) Updated');
