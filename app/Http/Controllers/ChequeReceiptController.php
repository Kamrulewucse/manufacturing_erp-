...
class ChequeReceiptController extends Controller
{
    public function datatable() {
        ...
        return DataTables::eloquent($query)
            ...
            ->addColumn('action', function(ReceiptPayment $receiptPayment) {
                $btn = '';
                $btn .= '<a target="_blank" href="'.route('cheque_receipt_print',['receiptPayment'=>$receiptPayment->id]).'" class="btn btn-dark btn-sm"><i class="fa fa-print"></i></a> <a href="'.route('cheque_receipt_details',['receiptPayment'=>$receiptPayment->id]).'" class="btn btn-dark btn-sm"><i class="fa fa-info-circle"></i></a> ';
                if ($receiptPayment->is_delete == 0)
                    $btn .= ' <a href="'.route('cheque_receipt.edit',['receiptPayment'=>$receiptPayment->id]).'" class="btn btn-dark btn-sm"><i class="fa fa-edit"></i></a>';
                return $btn;
            })
    ...
    {
        return view('accounts.cheque_receipt.all');
    }
    public function details(ReceiptPayment $receiptPayment)
    {
        $inWordAmount = new DecimalToWords();

        return view('accounts.cheque_receipt.details',compact('receiptPayment','inWordAmount'));
    }
    public function print(ReceiptPayment $receiptPayment)
    {
        $inWordAmount = new DecimalToWords();
        return view('accounts.cheque_receipt.print',compact('receiptPayment','inWordAmount'));
    }
    public function rangePrint(Request $request)
    {
        ...
        $from = 'BMR-'.$request->from.'-'.$selectBank->account_code;
        $to = 'BMR-'.$request->to.'-'.$selectBank->account_code;

        $voucherLists = [];
        for($i = $request->from;$i <= $request->to;$i++){
            array_push($voucherLists,'BMR-'.$i.'-'.$selectBank->account_code);
        }

        $receiptPayments = ReceiptPayment::where('payment_account_head_id', $request->bank_account_code)
        ...
        return view('accounts.cheque_receipt.range_print',compact('receiptPayments',
    ...
    public function create()
    {
        ...
        return view('accounts.cheque_receipt.add',compact('taxSections','projects'));
    }
    public function createPost(Request $request)
    {
        //dd($request->depositor);
        $rules = [
            ...
            'issuing_bank_name'=>'nullable|max:255',
            'issuing_branch_name'=>'nullable|max:255',
        ...
        if ($request->client_type == 1){
            ...
            $rules['depositor_name']= 'required|max:255';
        }
        ...
        if ($request->other_account_head_code){
            ...
            foreach ($request->other_account_head_code as $reqOtherAccountHeadCode){
                ...
                //Expense Other Head Amount credit
                $log = new TransactionLog();
                $log->payment_account_head_id = $receiptPayment->payment_account_head_id;
                $log->payee_depositor_account_head_id = $accountHead->id;
                ...
                //Expense Other Head Amount Debit
                $log = new TransactionLog();
                $log->payment_account_head_id = $receiptPayment->payment_account_head_id;
                $log->payee_depositor_account_head_id = $accountHead->id;
                $log->transaction_type = 36;
                $log->receipt_payment_no = $voucherNo;
                $log->receipt_payment_sl = $receiptPaymentNoSl;
                $log->financial_year = financialYear($request->financial_year);
                $log->other_head = 1;
        ...
        return redirect()->route('cheque_receipt_details',['receiptPayment'=>$receiptPayment->id])
    ...
    public function edit(ReceiptPayment $receiptPayment)
    {
        ...
        $taxSections =TaxSection::orderBy('sort')->get();
        return view('accounts.cheque_receipt.edit',compact('receiptPayment','fiscalYear',
        'taxSections'));
    }
